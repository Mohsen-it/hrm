...
<template>
    <AppLayout :title="t('attendance.monthly_page.title')">
        ...
        <div class="card p-4 mb-4 flex items-center gap-3 flex-wrap">
            ...
            <label class="flex items-center gap-2 text-[12px]">
                <span>{{ t('attendance.fields.month') }}</span>
                <select v-model.number="month" class="form-input max-w-[120px]">
        ...
        <div class="card p-4 mb-4">
            ...
            <div class="grid grid-cols-2 md:grid-cols-6 gap-2 text-[12px]">
                <div v-for="(v, k) in (kpis.by_status || {})" :key="k" class="p-2 rounded bg-[var(--color-surface-1)]">
            ...
            <table class="w-full text-right border-collapse text-[13px]">
                <thead>
                    <tr class="bg-[var(--color-surface-1)]">
                        <th class="px-2 py-2">{{ t('attendance.fields.date') }}</th>
                ...
                <tbody>
                    <tr v-for="d in breakdown" :key="d.date" class="border-b border-[var(--color-hairline)]">
                        <td class="px-2 py-2" dir="ltr">{{ d.date }}</td>
                        <td class="px-2 py-2">{{ d.present }}</td>
                        <td class="px-2 py-2">{{ d.absent }}</td>
                        <td class="px-2 py-2">{{ d.late }}</td>
                        <td class="px-2 py-2">{{ d.overtime_minutes }}</td>
                    </tr>
                    <tr v-if="breakdown.length === 0">
                        <td colspan="5" class="text-center text-[var(--color-ink-muted)] py-4">—</td>
                    </tr>
                </tbody>
            </table>
        </div>
