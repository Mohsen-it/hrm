...
function applyFilter(key, value) {
    ...
    router.get(route('fingerprint-devices.unregistered-employees'), { ...newFilters, page: 1 }, {
        preserveState: true,
        preserveScroll: true,
        replace: true,
...
function onExport() {
    ...
    window.location.href = route('fingerprint-devices.unregistered-employees.export', { ...props.filters, ...live });
}
...
usePageTitle(t('fingerprint_devices.unregistered_employees'));
</script>

<template>
    
        <PageHeader
            :title="t('fingerprint_devices.unregistered_employees')"
            :description="t('fingerprint_devices.unregistered_employees_description')"
        >
            <template #actions>
                <Button variant="secondary" icon="fas fa-file-excel" @click="onExport">
                    {{ t('common.export') }}
                </Button>
                <Button variant="secondary" icon="fas fa-arrow-right" :href="route('fingerprint-templates.index')">
                    {{ t('fingerprint_devices.templates') }}
                </Button>
            </template>
        </PageHeader>
                    ...
                    <div class="flex items-center gap-2">
                        <div class="flex items-center gap-1.5 px-3 py-1.5 bg-mistral-danger-bg rounded-lg">
                            ...
                            <span class="text-sm font-semibold text-mistral-danger">
                                {{ total }} {{ t('fingerprint_devices.unregistered_count_label') }}
                            </span>
                        </div>
                    </div>
                </div>
        ...
        <DataTable
            ...
            :data="employees"
            :filters="filters"
            :route-name="'fingerprint-devices.unregistered-employees'"
            :only="['employees', 'total', 'filters']"
            :empty-title="t('fingerprint_devices.no_unregistered_title')"
            :empty-description="t('fingerprint_devices.no_unregistered_description')"
            storage-key="unregistered-employees"
            @search="onSearch"
            @export="onExport"
        >
