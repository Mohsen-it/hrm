...
function submit() {
    ...
    errors.value = {};
    router.post(route('vacations.requests.update', props.request.id), form, {
        preserveScroll: true,
...
usePageTitle(t('vacations.edit_request'));
</script>

<template>
    
        <PageHeader :title="t('vacations.edit_request')" :description="request.user?.name">
            <template #actions>
                <Button variant="secondary" icon="fas fa-arrow-right rtl-flip" :href="route('vacations.requests.show', request.id)">{{ t('common.back') }}</Button>
            </template>
        </PageHeader>
        ...
        <form class="space-y-6" @submit.prevent="submit">
            ...
            <FormActions :save-label="t('common.update')" :cancel-label="t('common.cancel')" :cancel-href="route('vacations.requests.show', request.id)" :saving="processing" />
        </form>
    </template>
