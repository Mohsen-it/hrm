    ...
    <AppLayout :title="t('vacations.view_request')">
        <PageHeader :title="t('vacations.view_request')" :description="request.vacation_type?.name_ar">
            <template #actions>
                ...
                <Button v-if="request.status === 'pending'" variant="primary" icon="fas fa-pen" :href="route('vacations.my.edit', request.id)">
                    {{ t('common.edit') }}
                </Button>
            </template>
        </PageHeader>
        ...
        <Card variant="base" padding="none">
            <div class="p-5 sm:p-6">
                <div class="flex items-center gap-4 mb-6 pb-6 border-b border-mistral-hairline-soft">
                    <div class="w-16 h-16 rounded-xl bg-mistral-surface flex items-center justify-center border border-mistral-hairline-soft">
                        <i class="fas fa-calendar-check text-[28px] text-mistral-stone"></i>
                    </div>
                    <div>
                        <h2 class="text-[18px] font-semibold text-mistral-ink">{{ request.vacation_type?.name_ar }}</h2>
                        <p class="text-[13px] text-mistral-steel mt-1">{{ request.start_date }} — {{ request.end_date }}</p>
                        <div class="mt-2">
                ...
                <dl class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                    <div v-for="(field, idx) in fields" :key="idx" class="flex flex-col">
                        <dt class="text-[11px] font-semibold text-mistral-stone uppercase tracking-wider">{{ field.label }}</dt>
                        <dd class="text-[14px] text-mistral-ink mt-1 break-words">{{ field.value }}</dd>
                    </div>
                </dl>

                <div v-if="request.status === 'pending'" class="mt-6 pt-6 border-t border-mistral-hairline-soft">
                    <Button
                        variant="danger"
                        icon="fas fa-times"
